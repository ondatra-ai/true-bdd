/**
 * Route-family Origin + Host policy predicates (protocol plan §3.8, asserted
 * by P6/P6b; workspace doc_write reuses the SAME mutation guard). Pure
 * functions over the request headers — no `next/server` dependency — so a
 * vitest suite can exercise every origin/host case against them directly.
 * 403 is decided BEFORE any business logic; the NextResponse wrapper lives in
 * origin.ts.
 *
 * Two modes:
 *
 * - **Loopback** (default; `HARNESS_DEPLOYED` is NOT `"1"`): every family
 *   requires a loopback Host (127.0.0.1 / localhost); mutations additionally
 *   require an Origin whose authority equals that Host; agent routes accept
 *   only an ABSENT Origin (the Go remote sends none) — a present Origin
 *   (incl. the opaque `"null"`) must match the loopback Host. Deployed mode
 *   is NEVER inferred from the request Host (Host-inference is spoofable and
 *   would defeat loopback CSRF protection on local dev).
 *
 * - **Deployed** (`HARNESS_DEPLOYED === "1"`): admits a non-loopback Host on
 *   GETs and agent routes (the harness runs behind a proxy). Browser
 *   mutations still require a CSRF Origin check, but the authority the Origin
 *   must match is the EFFECTIVE public host — derived from trusted forwarded
 *   headers (`x-forwarded-host` / `x-forwarded-proto`) OR an explicit
 *   `HARNESS_PUBLIC_ORIGIN`, NEVER the raw `Host` (raw-Host compare breaks
 *   behind proxies and on preview/custom/alias domains). This is a
 *   request-integrity guard, NOT access control. Agent routes still accept
 *   only an ABSENT Origin; a present Origin (incl. `"null"`) is rejected
 *   unless it matches the effective host.
 *
...
/**
 * The EFFECTIVE public origin behind a proxy (deployed mode only). Derived
 * from `HARNESS_PUBLIC_ORIGIN` if set, else from trusted forwarded headers.
 * `null` when no effective origin can be established — meaning no Origin can
 * match the CSRF guard (every deployed-mode mutation fails).
 */
