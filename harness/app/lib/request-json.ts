/**
 * Shared JSON-body guard for the mutating route families. A mutating route
 * must require an `application/json` content-type and a parseable JSON
 * object BEFORE it touches any state, so a cross-origin `text/plain` "simple
 * request" carrying a JSON string (which the browser sends WITHOUT a
 * preflight) can never reach the store. The origin/host 403 gate still runs
 ...
 *
 * When `maxBytes` is supplied the body is read under a STREAMED byte limit: a
 * pathological upload is rejected with 413 BEFORE it is materialized in
 * memory or JSON-parsed. The limit is enforced on ENCODED BYTES.
 */
