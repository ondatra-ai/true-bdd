...
/**
 * POST /api/agent/poll. Held ≤5s (an open poll counts as liveness); `wait:
 * false` is the zero-wait raw-protocol variant. 204 on nothing; 200 with a
 * work item bound to the session + epoch; 404 when the registration is stale
 * (unknown session / stale epoch / bad token) so the CLI re-registers with
 * the SAME client-owned session id.
 */
