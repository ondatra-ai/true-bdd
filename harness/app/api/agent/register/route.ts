...
/**
 * The inventory-fit budget the remote fits its snapshot under (plan §1.5).
 * It is DISTINCT from the large streamed reply cap (reply_budget_bytes): a
 * per-server `MAX_INVENTORY_REQUEST_BYTES` can be set tiny to exercise the fit
 * ladder (snapshot_truncated / limit_too_small) WITHOUT 413-ing every read.
 * Absent ⇒ the reply cap (no inventory degradation).
 */
function inventoryBudgetBytes(): number {
  ...
  // Clamp to the actual reply cap: an oversized MAX_INVENTORY_REQUEST_BYTES must
  // never let the CLI fit a snapshot the relay would then have to 413-reject.
 ...
 * relay restart re-register into the SAME session; the fresh connection_epoch
