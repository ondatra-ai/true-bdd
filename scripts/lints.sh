...
# of at commit time.
#
# Selection is by extension, and a gate two files select still runs once:
#
#	*.go           golangci-lint on its package, comments, layout
#	*.sh           comments
#	*.yaml|*.yml   schemas, comments
#	CLAUDE.md      the CLAUDE.md gate (size, width, upstream mirror)
#	*.md           markdownlint — CLAUDE.md excepted, its own gate owns it
#	*.py           layout — the gate that refuses Python in this repo
...
if [ "$layout" = 1 ]; then
	./scripts/lint-layout.sh "$@" || status=1
fi
...
if [ "$claude_md" = 1 ]; then
	./scripts/lint-claude.md.sh || status=1
fi
