...
You are making failing end-to-end tests pass by writing PRODUCTION code. The failing
tests below are your ENTIRE specification — make them pass WITHOUT changing them. Do
EXACTLY the four steps and nothing else. You may write files ONLY under
...
# The failing tests (your whole spec — the reproduce block)
...
1. **Baseline → `01-baseline.md`.** Run the FULL suite (e2e + unit). Confirm the red
   set matches the reproduce block above. If the failing set differs, STOP: write
   `result.json` with `status: "DRIFT"` and the actual failing tail, and end.
...
3. **Verify green → `03-green-verify.md`.** Re-run the reproduce specs to green, then
