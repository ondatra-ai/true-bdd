...
# fast-forwarded to origin. docs/context/ is exempt from the reset — its
# uncommitted archivist appends are durable knowledge, and the closing sweep
# launched below re-appends to the ledgers right after this script anyway.
set -euo pipefail
...
("$ROOT/.claude/hooks/context.py" sweep </dev/null >/dev/null 2>&1 &)
