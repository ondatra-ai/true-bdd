...
**`--fix` refuses to start unless every prompt it would walk carries an `F:`.** The fix template reaches the VALIDATION turn, not the fix turn: the prompt template gates both the "If Validation Fails" section and the `fix_prompt:` output field on it, so a prompt without one is never asked for a fix_prompt — and the fix generator still spends a turn rendering "Suggested Fix Template:" followed by nothing, a cell that can only end at `CellFailedNoFix`. The check runs over the flattened prompt list (`skip:`-ped prompts already dropped), names every offender at once, and is inert without the flag. Consequence to know: `us create --fix` (9 prompts, 0 `F:`) and `us refine --fix` (12 prompts, 5 `F:`) refuse against the shipped checklists; `us apply`, `build tests` and `build code` are fully covered and unaffected. The five `us-refine-fix-*` fixtures still exercise the loop because their `checklist-prompts.yaml` narrows the tmpdir checklist to exactly the F-carrying prompts before the CLI is spawned.
...
- `docs/for_further/` — agreed designs for work not started yet, one Markdown file per idea. Tracked, and deliberately outside the doc universe: nothing under `documents:` or `paths:` names it, so `sync-doc-universe` does not audit it. Each file carries its own decision log, so the design can be picked up without the conversation that produced it.
- **`docs/*.html` are the published pages, and `.github/workflows/deploy-pages.yml` is the whole publish step.** `doc-universe.html` (also served as `index.html`) and `roadmap.html` are each self-contained — fonts and CSS inlined, no shared assets — so the workflow is a `cp` per page into `_site/`. Two consequences: **merging a change to one of them to `main` IS its deploy** (never publish one as a Claude artifact instead), and adding a third page means adding BOTH a `cp` line and a `paths:` trigger — a page missing from the `cp` list 404s rather than rendering unstyled, and one missing from `paths:` silently never redeploys. Both are repo-local like `for_further/`: nothing under `documents:` or `paths:` names them, so `sync-doc-universe` does not audit them. `docs/roadmap.md` is `roadmap.html`'s Markdown twin, transcribed from the ClickUp "Idea" whiteboard, which stays the source.
