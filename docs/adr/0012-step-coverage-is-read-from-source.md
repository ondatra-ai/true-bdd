...
which every suite answered with JSON in `$TRUEBDD_COVERAGE_REPORT_DIR`. The
engine now resolves it itself, by parsing `<suite root>/steps/`, and
...
The assembly is real. It is also **constant-expression** assembly. Measured
when this was decided, across all 466 `suite.Step(` call sites this repository
then carried — most of them in a second, browser-driving suite it has since
removed — every first argument was a string literal, or literals joined by `+`
with one of 19 identifiers, and every one of those a package-level `const` in
the same package. No `fmt.Sprintf`, no `strings.Join`, no loops, no cross-package
...
skipped, not an error." Pointed at that second suite's `steps/`, it dropped the
131 registrations composed from a shared selector constant and reported every
step they bind as unbound — a false gap, which looks exactly like real work to a
fix turn. A host that composes its patterns the same way gets the same false
gap; that, not this repository's present tree, is what the contract is for.
...
The proof that the mirror is faithful is cheap and was run: with both suites of
the day green, the compiled guard and the engine's scan agreed on all 299
scenarios then in the registry.
...
have, and a suite can fan out from `Register` across a hundred files through
some eighty helpers, as the removed one did. Today the two rules agree — no `register…` helper
...
is the migration notice, exactly as the environment variable's rename was under
the transport this replaces — `gopkg.in/yaml.v3` decodes non-strictly and would have ignored the key
in silence.
...
- CI loses its cheap red/green on unbound steps: `TestStepCoverage` is gone from
  the suites and from the gate line, so an unbound step now surfaces when
...
- Three new startup refusals, each pinned by a registry scenario: a pattern that
  ...
  narrowing itself against a host declaring no command at all (E2E-298). The
  transport this replaces closed with no scenario covering the ask; that gap is
  closed here.
