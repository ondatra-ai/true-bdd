...
and handed to the postmortem, under the §6 span names wherever that tree names
the operation (`split` and `dispose` are new). Everything else is still design
— no OTel SDK, no `otel-cli`, no OTLP, no store, no UI, and none of the §5
identity levels.
