 ...
 * gold-standard screenshots (tests/bdd-web/goldens/<name>.png).
 *
 ...
 * prototype (services/bdd-web/design/proto-workspace/app, booted via `next dev`) ONLY
...
test.describe("design goldens capture (authoring-time)", () => {
  ...
  test("capture all design gold standards from the booted prototype", async ({ page }, testInfo) => {
    ...
    const manifest = {
      ...
      source: "services/bdd-web/design/proto-workspace/app (booted via `next dev` by helpers/proto-baseline.ts)",
