 ...
 * `services/bdd-web/design/`: `system/tokens.css` (the S&F
 ...
 *     concrete named layout checks derived from `services/bdd-web/design/SPEC.md`.
 *
...
// ── Design-source locations (all under services/bdd-web/design/) ──

export const REPO_ROOT = findRepoRoot();
export const DESIGN_ROOT = path.join(REPO_ROOT, "services", "bdd-web", "design");
...
// tests/bdd-web/goldens/, captured from the booted prototype at test-AUTHORING
...
export const GOLDENS_DIR = path.join(REPO_ROOT, "tests", "bdd-web", "goldens");
  ...
  if (!fs.existsSync(golden)) {
    throw new Error(
      `design golden missing: ${golden}\n` +
        ...
        "run `npm run goldens:update` in tests/bdd-web after writing or changing a visual spec.",
    );
  }
...
export function requireGoldenCropMeta(name: string): GoldenCropMeta {
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new Error(`goldens manifest missing: ${manifestPath} — run \`npm run goldens:update\` in tests/bdd-web.`);
  }
  ...
  if (meta === undefined) {
    throw new Error(
      `goldens manifest has no crop metadata for "${name}" — run \`npm run goldens:update\` in tests/bdd-web ` +
...
function canvasParityRubric(): string {
  return [
    ...
    "",
    "The mockup's overview is a DESIGN SHOWCASE that renders the '05 — degraded &",
    "flagged states' panel (a stack of warning/alert cards between the header and",
    "the inventory-health list) to demonstrate every degraded state at once.",
    "Production surfaces those banners ONLY for a genuinely degraded session, so a",
    "healthy session correctly omits the panel. This panel's PRESENCE OR ABSENCE",
    "is data-conditional: TOLERATE it entirely — never fail a check, and never",
    "report an other_difference, about the degraded/flagged-states panel itself or",
    "about the vertical-spacing / section-count change caused by its absence.",
    "",
    "Evaluate exactly these named checks and return the schema-forced JSON verdict:",
...
function sessionsParityRubric(): string {
  return [
    ...
    "",
    "TOLERATE one known, intentional divergence — it must NEVER fail a check:",
    "  production may show an empty-state or unavailable message the baseline lacks.",
    ...
    "values AND the tolerated divergence above) that no named check covers. severity",
