 ...
 * (tests/materializer — see its doc.go for the CLI contract,
...
/** Resolves a fixture name to `tests/harness/fixtures/<name>`. */
export function fixtureDir(name: string): string {
  return path.join(suiteContext().repoRoot, "tests", "harness", "fixtures", name);
}
 ...
 * preparation failure.
 *
 * @param nameOrDir fixture name (resolved against tests/harness/fixtures)
