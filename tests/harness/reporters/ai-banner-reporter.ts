...
export default class AiBannerReporter implements Reporter {
  ...
  private suiteDir = process.cwd();

  onBegin(config: FullConfig): void {
    // config.rootDir is the resolved testDir; the report belongs in the
    // E2E suite's own gitignored test-results/ (the config file's dir).
  ...
  private writeReport(
    ...
    const outDir = path.join(this.suiteDir, "test-results");
