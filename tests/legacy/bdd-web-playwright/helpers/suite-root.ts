 ...
 *
 *   <suiteRoot>/bin/true-bdd       — `go build -o ... ./services/bdd-cli`
 *   <suiteRoot>/bin/materializer   — `go build -o ... ./tests/libraries/materializer`
 *
...
export async function createSuiteContext(): Promise<SuiteContext> {
  ...
  const harnessDir = path.join(repoRoot, "services", "bdd-web");
  ...
  await execFileAsync("go", ["build", "-o", binPath, "./services/bdd-cli"], {
    cwd: repoRoot,
  });
  await execFileAsync(
    "go",
    ["build", "-o", materializerBin, "./tests/libraries/materializer"],
    { cwd: repoRoot },
  );
...
export function suiteContext(): SuiteContext {
  ...
  return {
    suiteRoot,
    repoRoot,
    harnessDir: path.join(repoRoot, "services", "bdd-web"),
    binPath,
    materializerBin,
    standaloneBundle,
  };
