 ...
 *
 * This suite is a self-contained package under `tests/bdd-web/` (its own
 * package.json + node_modules); the config sits at the suite root, so every
 * path below is relative to `tests/bdd-web/`.
 *
...
export default defineConfig({
  ...
  projects: [
      ...
      : []),
    {
      ...
      // screenshots from the booted prototype (tests/bdd-web/goldens/). The
