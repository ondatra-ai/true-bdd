...
// commandVersion is the version-probe command name (plan §3.1). Named so
// the mapping and the dispatch tests share one source of truth.
const commandVersion = "version"

// commandArgs maps a dispatched run's typed command to the CLI argument
// vector the remote spawns its own binary with (plan §3.2). The server's
