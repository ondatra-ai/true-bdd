...
> (`services/bdd-web/design/proto-workspace/`) superseded it as the per-screen design
...
Historically under `services/bdd-web/design/mockups/`:
...
- **Architecture** (`service.html`, `vocabulary.html`, sidebar Architecture
  tree) ← `tests/bdd-web/fixtures/a1-create-no-fix/input/docs/architecture/architecture.yaml`
...
- **Requirements/Scenarios** (`scenarios.html`, `scenario-detail.html`,
  sidebar flat list) ←
  `tests/bdd-web/fixtures/p3-inventory-spread/input/docs/scenarios.yaml`
  (E2E-601/602/603, service + linked stories, merged given/when/then) and
  `tests/bdd-web/fixtures/a5-build-tests-fix/input/docs/scenarios.yaml`
  (INT-901).
- **Ops surfaces** (`runs.html`, `run-detail.html`, prompt dialogs) — session
  ...
  `tests/bdd-web/helpers/README-testids.md` (`state`/`outcome` enums).

## 8. S&F design-system mirror

`services/bdd-web/design/system/` is the offline S&F mirror (tokens, fonts,
